...
void eliminarProveedor(vector<Proveedor> &listaProveedores) {
    ...
    cout << "DAR DE BAJA PROVEEDOR" << endl;
    {
        ...
        for(vector<Proveedor>::iterator it = listaProveedores.begin(); it != listaProveedores.end(); ++it){
            if(idBuscar == it->getID()){ //En cada iteracion, se verifica si el idProveedor del elemento actual coincide con idBuscar
                listaProveedores.erase(it);
                cout << "Proveedor eliminado exitosamente." << endl;
                encontrado = true; //Si es asi, se elimina el proveedor de la lista y se establece la bandera encontrado
                break;
            }
        }
                ...
                case 3:
                    ...
                    if(listaProveedores.empty()) {
                        ...
                        for (vector<Proveedor>::size_type i = 0; i < listaProveedores.size(); ++i) {
                            cout << endl << listaProveedores.at(i) << endl; //Se usa el metodo .at() porque realiza comprobaciones exactas del tamanio del vector
                        }
                    }
                    system("pause");
                    break;
...
void agregarProductoMenu(Inventario &inventario) {
    ...
    // Crear un nuevo Producto en el heap y almacenar un puntero a el
...
void pedidos(vector<Pedido> &listaPedidos) {
    ...
    cout << "Cliente Luis - Producto: Silla Gamer Corsair T3 Rush Gaming 2023 Carbon / 2023 Edition" << endl;
    ...
    pedidos_exclusivos.push_back(PedidoExclusivo(104, "Mario", 87999.0, "Envio prioritario", "Computadora PRIDE GAMING PBA SHIMU / NVIDIA GeForce RTX 4090 / INTEL CORE i9-14900KF / 32GB RAM / 2TB SSD M.2 NVMe / HDD 2TB / ENF. LIQ. 360MM RGB / 1300W 80+ PLATINUM / Gratis juego Star Wars Outlaws"));
